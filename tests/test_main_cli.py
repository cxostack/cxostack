...
def test_cli_cto_calls_cmd_cto():
    with patch("sys.argv", ["main.py", "cto", "idea", "my saas idea"]), \
...
def test_cli_mode_respected():
    with patch("sys.argv", ["main.py", "--mode", "best", "cto", "idea", "my idea"]), \
         patch("main.cmd_cto") as mock:
        main._cli_mode()
...
def test_cli_cto_continue():
    with patch("sys.argv", ["main.py", "cto", "continue", "phase-2"]), \
