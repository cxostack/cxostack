...
def test_dispatch_cheap_calls_groq(tmp_path):
    ...
    with patch("model_router.get_model", return_value="groq/llama-3.1-8b-instant"), \
         patch("tools.groq_relay.call_groq", return_value="def foo(): pass") as mock_groq, \
...
def test_dispatch_avg_calls_claude():
    ...
    with patch("model_router.get_model", return_value="claude-sonnet-4-6"), \
